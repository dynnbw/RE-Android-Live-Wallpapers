/*
 * MoonCalculator.parallacticAngleDeg 的约束测试 —— 纯 JVM:
 *
 *   javac -d /tmp/partest \
 *         app/src/main/java/com/reandroid/wallpaper/grass/MoonCalculator.java \
 *         tools/grass-test/ParallacticAngleTest.java
 *   java -cp /tmp/partest com.reandroid.wallpaper.grass.ParallacticAngleTest
 *
 * 这个角决定月面相对屏幕要转多少（整体旋转：纹理和终止线一起转）。
...
public final class ParallacticAngleTest {

    private static int failures = 0;

    public static void main(String[] args) {
        testCulminationIsUpright();
        testNorthOfZenithFlips();
        testKnownValue();
        testRiseSetMirror();
        testLargestNearHorizon();
        testRange();
    ...
    private static void testCulminationIsUpright() {
    ...
    private static void testRiseSetMirror() {
    ...
    /** 接近地平线时偏得最多；中天时最小（0）。 */
    private static void testLargestNearHorizon() {
        ...
        System.out.println("中天 |q| = " + atMeridian + "，近地平线 |q| = " + nearHorizon);
    }
            ...
            for (double dec = -28; dec <= 28; dec += 7) {
                for (double h = -180; h <= 180; h += 17) {
                    ...
                    if (Double.isNaN(q) || Double.isInfinite(q)) {
                        failures++;
                        System.out.println("失败: (" + lat + "," + dec + "," + h + ") 得到 " + q);
                        continue;
                    }
                    // ±180 对旋转角是等价的（H=±180 时天体在地平线下，本来也不可见）
                    if (q < -180.0 || q > 180.0) {
                        failures++;
                        System.out.println("失败: (" + lat + "," + dec + "," + h + ") 越界 " + q);
                    }
                }
            }
        }
    ...
    private static void assertEquals(String name, double expect, double actual, double eps) {
