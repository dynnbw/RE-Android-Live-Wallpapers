    ...
    private class GLESEngine extends Engine implements Runnable {
        ...
        public void run() {
            ...
            try {
                ...
                // 优先带深度的配置，没有就退回不带深度的。
                EGLConfig config = GlCapabilities.chooseConfig(display, true);
                if (config == null) config = GlCapabilities.chooseConfig(display, false);
                if (config == null) {
                ...
                context = GlCapabilities.createContext(display, config);
