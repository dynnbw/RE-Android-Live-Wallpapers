...
final class FallScene {
    ...
    /**
     * 闪光贴在波前后面多宽（网格单位）。spread 每秒走 30 单位，所以约 0.1 秒。
     *
     * <p>**这个值就是环的宽度。** 收到 3 之前是 5，上机看环太胖、不像环
     * （用户的原话是「收窄一些，让他更像圆环一些」）。
     * 顺带也更接近实测：甲藻的闪光只有 50~150ms，5 单位那版是 0.17 秒，偏长。
     *
     * <p>还想更细就继续调小；再往细里走要靠辉光的半径一起收，否则光晕会把环糊胖。
     */
    static final float ALGAE_BAND = 3.0f;
    /**
     * 蓝藻打在叶子上的**光色**（增益已经乘进去）。
     *
