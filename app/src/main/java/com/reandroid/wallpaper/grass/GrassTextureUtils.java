...
final class GrassTextureUtils {
     ...
     * 水花的贴图：一颗小的亮点。
     *
     * <p><b>不能复用挂珠那张</b> —— 那张的珠体是刻意几乎全透明的（好让背后的草透出来），
     * 只有边缘和高光是实的。拿它去画 3~7 像素的水花，等于什么都没画 ——
     * 这正是"看得到挂珠、看不到飞溅"的原因。
     */
    static int createWaterSplashTexture(int size) {
        int n = size * size;
        byte[] rgba = new byte[n * 4];
        float r = size * 0.5f;
        for (int y = 0; y < size; y++) {
            for (int x = 0; x < size; x++) {
                ...
                // 实心的芯 + 柔和的边，整颗都亮 —— 与水珠贴图正好相反
                float core = smoothstep(1.0f, 0.55f, d);
                float glow = smoothstep(1.0f, 0.0f, d) * 0.35f;
                float alpha = clamp01(core + glow);
                if (alpha <= 0.0f) {
                    continue;
                }
                int o = (y * size + x) * 4;
                // 略微偏冷的水色
                rgba[o] = toByte(0.88f);
                rgba[o + 1] = toByte(0.94f);
                rgba[o + 2] = toByte(1.00f);
                rgba[o + 3] = toByte(alpha);
            }
        }
