...
final class GrassBacklight {
     ...
     */
    static float strength(float sunAltitudeDeg, boolean moonUp, float moonAltitudeDeg) {
        float sun = smoothstep(-FADE_DEG, 0.0f, sunAltitudeDeg);
        float moon = moonUp ? smoothstep(-FADE_DEG, 0.0f, moonAltitudeDeg) : 0.0f;
     ...
     */
    static float highlight(float sunAltitudeDeg, boolean moonUp, float moonAltitudeDeg) {
        ...
        float moon = moonUp
                ? smoothstep(-FADE_DEG, 0.0f, moonAltitudeDeg) * (1.0f - rise)
                : 0.0f;
     ...
     */
    static boolean sourceIsMoon(float sunAltitudeDeg, boolean moonUp, float moonAltitudeDeg) {
        if (!moonUp) {
            return false;
        }
    ...
    static float effectiveStrength(boolean enabled, float sunAltitudeDeg,
                                   boolean moonUp, float moonAltitudeDeg,
                                   WeatherCondition condition) {
        if (!enabled) {
            return 0.0f;
        }
        return strength(sunAltitudeDeg, moonUp, moonAltitudeDeg) * weatherScale(condition);
    }
