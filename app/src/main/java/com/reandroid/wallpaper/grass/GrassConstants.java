...
final class GrassConstants {
     ...
     *
     * <p>0.1 是上机调下来的：只要不是纯黑、还能透出一点本色就够，再高夜里会发亮。
     */
    static final float GRASS_NIGHT_VALUE_FLOOR = 0.1f;
    ...
    //
    //   上边 —— **白日天空不能发。** SKY_FIELD_DAY 最亮的一端是 #7EBCEB，明度 0.686，
    //   阈值高过它就一定不糊；0.75 留了 0.06 的余量。
    //
    //   下边 —— **月亮、星星、萤火虫要发。** 它们本身就是光源，有辉光是物理正确的。
    //   原来定 0.85 就是为了把它们挡掉，那是拿一个正确的现象去迁就一个偏高的数。
    //
    // 取值是上机调下来的，观感不对就调这里。

    /** 亮度阈值。低于它的不发辉光。上下两条线的位置见上面的注释。 */
    static final float GLOW_THRESHOLD = 0.75f;
