...
final class GrassAstronomyCalculator {
    ...
    static SolarEclipse computeSolarEclipse(MoonCalculator.MoonData data, Calendar now,
        ...
        float moonAltRad = (float) Math.toRadians(MathUtils.clamp((float) data.moonAltitudeDeg, 0.0f, 90.0f));
        ...
        float solarVisible = MathUtils.smoothStep(-0.5f, 5.0f, (float) data.sunAltitudeDeg);
        float lunarVisible = MathUtils.smoothStep(-2.0f, 4.0f, (float) data.moonAltitudeDeg);
        float horizonAttenuation = MathUtils.smoothStep(1.5f, 12.0f,
                (float) Math.min(data.sunAltitudeDeg, data.moonAltitudeDeg));
        float conjunctionAttenuation = 1.0f - MathUtils.smoothStep(8.0f, 22.0f, phaseDelta);
        float nodeAttenuation = 1.0f - MathUtils.smoothStep(3.0f, 9.0f, nodeLatitude);
        ...
        float phaseProgress = MathUtils.clamp((signedAzDelta + 12.0f) / 24.0f, 0.0f, 1.0f);
    ...
    static MoonEclipse computeMoonEclipse(MoonCalculator.MoonData data) {
        ...
        float pen = MathUtils.clamp((float) penumbra, 0.0f, 1.0f);
        float umb = MathUtils.clamp((float) umbra, 0.0f, 1.0f);
        float total = MathUtils.clamp((float) (umbra - 1.0), 0.0f, 1.0f);
        ...
        float easedPhase = MathUtils.smoothStep(0.0f, 1.0f, phase);
        float offsetMag = MathUtils.mix(1.6f, -1.6f, easedPhase);
        ...
        if (penumbra > 0.0) return new MoonEclipse(1, MathUtils.clamp((float) penumbra, 0.0f, 1.0f), phase, offX, 0.0f);
    ...
    static float clockProgress(float now, float start, float end) {
        ...
        return MathUtils.clamp(elapsed / duration, 0.0f, 1.0f);
    }
    ...
    private static float[] computeApparentAngularRadii(Calendar now, float solarRadiusMeanDeg,
        ...
        sunRadiusDeg = MathUtils.clamp(sunRadiusDeg, 0.258f, 0.275f);
        moonRadiusDeg = MathUtils.clamp(moonRadiusDeg, 0.245f, 0.285f);
    ...
    private static float computeDiscOverlapFraction(float sunRadiusDeg, float moonRadiusDeg, float centerDistanceDeg) {
        ...
        return MathUtils.clamp((float) (overlapArea / sunArea), 0.0f, 1.0f);
    }
