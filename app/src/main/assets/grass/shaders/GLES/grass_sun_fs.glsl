...
vec4 getWeatherEffect(vec2 uv) {
    ...
    // 参考实现在这里还乘一个"地平线淡出"，把画面下半部压暗。**搬过来是错的。**
    //
    // 它在参考的卡片场景里无害 —— 那颗太阳基本不落到下半屏。但 grass 把高度角直接映射成
    // 屏幕 y（sunY = height * (1 - alt/90)），alt=0 时太阳就在屏幕底缘。于是"压暗下半屏"
    // 变成了"太阳越低越不可见"：实测系数 alt=0 → 0、18° → 0.26、36° → 0.74，
    // 要到 **54° 才满**。整段日落都在被它抹掉，表现就是"地平线附近发虚"。
    //
    // 这里不需要淡出：太阳往下走会自然滑出屏幕下缘，而草叶本来就画在它上面。
