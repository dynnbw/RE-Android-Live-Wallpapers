...
// 精度必须与 silk_vs.glsl 的对应 out 一致，见那边的说明。
in mediump float v_fragmentColor;
in mediump vec2 v_coord;
in mediump vec4 localPos;
in mediump float flash;
void main() {
    ...
    vec4 dot = texture(CC_Texture0, v_coord);
    fragColor = (v_fragmentColor * dot + (1.0 - v_fragmentColor) * uOriginColor) * uOriginAlpha * alpha * flash;
    ...
    safe *= step(0.0, fragColor.r) * step(0.0, fragColor.g);
    safe *= step(0.0, fragColor.b) * step(0.0, fragColor.a);
    if (safe < 0.5) {
        fragColor = vec4(0.0);
    }
}
